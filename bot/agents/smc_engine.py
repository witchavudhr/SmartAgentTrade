...
class SMCEngine:
    ...
    def _find_order_blocks(self, df: pd.DataFrame, structures: list = None):
        ...
        # mitigated เลยจริงๆ — mitigation (low เทียบ bottom เป๊ะๆ จาก MT5 OHLC จริง)
        # คือขอบเขตที่แม่นอยู่แล้ว ไม่ต้อง cap จำนวนซ้อนทับเข้าไปอีกชั้น เก็บ
        # unmitigated ไว้ทั้งหมดไม่จำกัด แค่ mitigated แล้วค่อย cap ไว้อ้างอิงบริบท
        _bulls_all = [o for o in obs if o.kind == 'bullish']
        _bears_all = [o for o in obs if o.kind == 'bearish']
        bulls = [o for o in _bulls_all if not o.mitigated] + [o for o in _bulls_all if o.mitigated][-3:]
        bears = [o for o in _bears_all if not o.mitigated] + [o for o in _bears_all if o.mitigated][-3:]
        return bulls + bears
...
def get_session() -> dict:
    ...
    return {
        "session": name,
        "emoji": emoji,
        "tradeable": (6.0 <= t < 19.0) and not market_closed,
